    ...
    def fill_attendance():
        ...
        try:
            ...
            connection = pymysql.connect(
                host='localhost', user='root', password='mysql@143', db='manually_fill_attendance')
        ...
        if subb == '':
            ...
            def create_csv():
                ...
                csv_name = '"E:/Attendance Management System using Face Recognition/Attendance/Manually Attendance"'+DB_table_name+'.csv'
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                ts = time.time()
                date = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')  # Current date (optional if needed)
                timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')  # Current time (optional if needed)

                base_path = "E:/Attendance Management System using Face Recognition/Attendance/"

                # Construct the file path
                fileName = os.path.join(base_path, f"{Subject}.csv")
                ...
                if os.path.exists(fileName):
                    ...
                    # Drop duplicates based on 'Enrollment'
                    # updated_attendance = updated_attendance.drop_duplicates(['Enrollment'], keep='first')
                    
                    # Save the updated attendance back to the same file
                    updated_attendance.to_csv(fileName, index=False)
                    

                    print(f"Updated existing file: {fileName}")
                else:
                    # If file doesn't exist, save the new attendance as a new file
                ...
                Hour = current_time.strftime('%H')  # Hour in 24-hour format (00-23)
                Minute = current_time.strftime('%M')  # Minute (00-59)
                Second = current_time.strftime('%S')  # Second (00-59)
                ...
                try:
                    global cursor
                    connection = pymysql.connect(
                        host='localhost', user='root', password='mysql@143', db='face_reco_fill')
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
    ...
    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"E:/Attendance Management System using Face Recognition/Attendance/-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'pragya':
            if password == 'pragya123':
                ...
                cs = 'E:/Attendance Management System using Face Recognition/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
